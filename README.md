...
# Process data, build warehouse, and view reports
uv run python src/analytics_project/data_prep.py
uv run python src/analytics_project/etl_to_dw.py
# Open data/dw/smart_sales.pbix in Power BI Desktop to view analytics
```
...
- 📈 **Power BI Reporting**: Interactive Dashboards, OLAP Operations
...
- Git
- VS Code (recommended)
- Power BI Desktop (for report viewing)
...
### 3.7 View Power BI Reports

After building the data warehouse, explore your Power BI visuals:

1. Open `data/dw/smart_sales.pbix` in Power BI Desktop
2. Refresh the data connection to load latest results
3. Interact with your OLAP operations:
   - Use slicing filters to focus on specific date ranges
   - Explore dicing operations in matrix visuals
   - Drill down through time hierarchies
4. Analyze the business insights generated by your visuals

```
Location: data/dw/smart_sales.pbix
Data Source: data/dw/smart_sales.db
```

**Key Visualizations:**
- **OLAP Operations**: Slicing, dicing, and drilldown capabilities
- **Sales Analytics**: Customer segmentation and regional performance
- **Product Intelligence**: Category-level sales and trends
- **Temporal Analysis**: Seasonal patterns and growth trajectory

### 3.8 Git add-commit-push to GitHub
...
### 3.10 Data Development Workflow
...
- **Power BI Report**: `data/dw/smart_sales.pbix` (located alongside the database)
- **Data Source**: SQLite database at `data/dw/smart_sales.db`
- **Connection**: ODBC DSN "SmartSalesDSN"

### Accessing the Report
1. Navigate to `data/dw/` directory
2. Open `smart_sales.pbix` in Power BI Desktop
3. Ensure the SmartSalesDSN is configured on your machine
4. Refresh data connection if needed to load latest data

### Visual Examples

![Dicing Operation](docs/images/dicing_example.png)
*Matrix visual demonstrating dicing operation across multiple dimensions*

![Drilldown Operation](docs/images/drilldown_example.png)
*Hierarchical chart showing drilldown capability through time levels*

![Slicing Operation](docs/images/slicing_example.png)
*Interactive slicers demonstrating filtering and slicing functionality*

### Key Visualizations Implemented

**OLAP Operations Dashboard:**
- **Slicing**: Date range filter for interactive temporal analysis
- **Dicing**: Matrix visual for Product Category × Customer Region analysis
- **Drilldown**: Time hierarchy (Year → Quarter → Month) in column charts
- **Top Customers**: Bar chart showing highest-spending customers
- **Sales Trends**: Line chart displaying temporal sales patterns

### Business Insights Generated
- **Customer Analytics**: High-value customer identification and segmentation
- **Regional Performance**: Market comparison across different regions
- **Product Intelligence**: Category-level sales performance and trends
- **Temporal Analysis**: Seasonal patterns and growth trajectory

### Technical Implementation
- **Data Model**: Star schema relationships between fact and dimension tables
- **Date Processing**: Power Query transformations for SQLite date handling
- **Interactive Features**: Drill-through, filtering, and cross-highlighting capabilities
...
Raw CSV Files → Data Preparation → Cleaned Data → ETL Process → Data Warehouse → Power BI Analytics
...
data/
├── dw/                       # Data warehouse database and Power BI reports
│   ├── smart_sales.db        # SQLite database
│   └── smart_sales.pbix      # Power BI report file
